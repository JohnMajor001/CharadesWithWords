  ...
  // Check if array needs to be repopulated
    if(catToUse.array.length == 2) {
      for(i = 0; i < catToUse.backUpArray.length; i++) {
       ...
       if(catToUse.easyArray.length == 2) {
      for(i = 0; i < catToUse.easyBackUpArray.length; i++) {
        catToUse.easyArray.push(easyBackUpArray[i]);
        }
      }
      if(ifEasy) {
        var whichArray = catToUse.easyArray;
      } else {
        var whichArray = catToUse.array;
      }
  var randomIndex = Math.floor(Math.random() * whichArray.length);
  var word = whichArray[randomIndex];
...
function roundEnds() {
  ...
  var drinkCatToUseNumber = (currentTeam.position-1)%categories.length;
  var drinkCatToUse = categories[drinkCatToUseNumber];
  // increment position
  ...
  if(drinkRules) {
    ...
    // var drinksArray = drinkCatToUse.drinkingLines;
...
function gotIt() {
  ...
  if(wordBox!=null) {
    ...
    if(catToUse.array.length == 2) {
      for(i = 0; i < catToUse.backUpArray.length; i++) {
        catToUse.array.push(catToUse.backUpArray[i]);
        }
      }
      if(catToUse.easyArray.length == 2) {
      for(i = 0; i < catToUse.easyBackUpArray.length; i++) {
        catToUse.easyArray.push(catToUse.easyBackUpArray[i]);
        }
      }
    var randomIndex = Math.floor(Math.random() * whichArray.length);
